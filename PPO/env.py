...
class Monitor:
    def __init__(self, width, height, saved_path):
        self.pipe = None # <--- Inizializza a None per sicurezza
        
        # Se non c'è saved_path, non ha senso creare il monitor,
        # ma assumiamo che saved_path esista se l'oggetto Monitor viene creato.
        
        self.command = ["ffmpeg", "-y", "-f", "rawvideo", "-vcodec", "rawvideo", "-s", "{}X{}".format(width, height),
                        "-pix_fmt", "rgb24", "-r", "60", "-i", "-", "-an", "-vcodec", "mpeg4", saved_path]
        try:
            # Tenta di aprire la pipe
            self.pipe = sp.Popen(self.command, stdin=sp.PIPE, stderr=sp.PIPE)
            print(f"Avvio registrazione video con FFmpeg: {saved_path}") # Messaggio di successo
        except FileNotFoundError:
            print("ATTENZIONE: FFmpeg non trovato. La registrazione video sarà disabilitata.")
            # self.pipe rimane None
        except Exception as e:
            print(f"ATTENZIONE: Errore nell'avvio di FFmpeg: {e}. La registrazione video sarà disabilitata.")
            # self.pipe rimane None

    def record(self, image_array):
        # Aggiungi questo controllo: se la pipe non è stata aperta, non tentare di scrivere.
        if self.pipe: 
            try:
                self.pipe.stdin.write(image_array.tostring())
            except Exception as e:
                # Gestisce eventuali errori di scrittura dopo l'avvio
                print(f"ATTENZIONE: Errore durante la scrittura nella pipe di FFmpeg: {e}")
                self.pipe = None # Disabilita ulteriori tentativi
                
    def close(self):
        # Aggiungi un metodo close per chiudere correttamente FFmpeg
        if self.pipe:
